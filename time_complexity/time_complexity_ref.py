...
def evaluate_sha3_256(repeat: int, count: int, byte_size: int = 32, increase: int = 10):
    timer = Timer()
...
def evaluate_recover_key(repeat: int, count: int):
    ...
    compressed_sig: bytes = signature[:33]
    print(f"msg_hash size:{len(msg_hash)}")
    print(f"sig size:{len(signature)}")
    for _ in range(count):
        print("uncompressed")
        timer.start()
        for _ in range(repeat):
        ...
        timer.start()
        for _ in range(repeat):
            recover_key(msg_hash, compressed_sig, True)
        timer.stop()
        print(timer)
